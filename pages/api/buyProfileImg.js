...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const response4 = await db.collection('history').insertOne({code: code, userId: userId, balance: balance, type: 'withDrawal', amount: price, date: new Date(),expiresAfter: new Date(), name: '프로필 구입'})
