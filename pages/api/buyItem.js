...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const response4 = await db.collection('history').insertOne({code: code, userId: userId,balance: balance, type: 'withDrawal', amount: itemData.itemPrice, date: new Date(),expiresAfter: new Date(), name: itemData.itemName + ' 구입'})
