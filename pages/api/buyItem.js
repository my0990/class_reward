...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const response2 = await db.collection('user_data').updateOne({userId: userId},{$push: {itemList: {itemName: itemData.itemName, itemPrice: itemData.itemPrice, itemId: itemId,  itemEmoji: itemData.emoji, itemExplanation: itemData.itemExplanation}}, $inc: {money: -itemData.itemPrice}}, {upsert: true})
