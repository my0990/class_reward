...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const response5 = await db.collection('class_data').findOne({ ...scopeFilter });
    ...
    const response = await db.collection('class_data').updateOne({ ...scopeFilter, "itemList.itemId": ItemId }, { $inc: { 'itemList.$.itemStock': -1 } })
    ...
    const response4 = await db.collection('history').insertOne({ ...scopeFilter, userId: userId, balance: money - itemData.itemPrice, type: '출금', amount: itemData.itemPrice, date: new Date(), expiresAfter: new Date(), name: itemData.itemName + ' 구입' })
