...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { itemData, userId, balance, classId } = req.body;
    ...
    const response = await db.collection('class_data').updateOne({ teacher_id: ObjectId.createFromHexString(token.user.teacher_id), classId: ObjectId.createFromHexString(classId), "itemList.itemId": ItemId }, { $inc: { 'itemList.$.itemStock': -1 } })
    ...
    const response4 = await db.collection('history').insertOne({ teacher_id: ObjectId.createFromHexString(token.user.teacher_id), classId: ObjectId.createFromHexString(classId), userId: userId, balance: money - itemData.itemPrice, type: '출금', amount: itemData.itemPrice, date: new Date(), expiresAfter: new Date(), name: itemData.itemName + ' 구입' })
