...
export default function DeleteModal({ pickedItem, currencyName, classId }) {
    ...
    const onSetItemStock = (e) => {
        ...
        if (isLoading) {
            ...
            fetch("/api/setItemStock", {
                method: "POST",
                body: JSON.stringify({ updatedItemStock: itemStock, itemId: pickedItem.itemId, classId: classId }),
                headers: {
                    "Content-Type": "application/json",
                },
            }).then((res) => res.json()).then((data) => {
