export default function CharacterCard(props) {
    const { user, startExp, commonDifference, ...domProps } = props;
  
    const findLargestSumUnderTarget = () => {
      let k = Math.floor(
        (-2 * startExp +
          commonDifference +
          Math.sqrt((2 * startExp - commonDifference) ** 2 + 8 * commonDifference * user.exp)) /
          (2 * commonDifference)
      );
    ...
    return (
      <div>
        <div
          {...domProps}
          className="w-[160px] p-[16px] m-[8px] bg-orange-200 rounded-xl cursor-pointer"
        >
          <div className="flex justify-between mb-[8px]">
            ...
            <div className="w-[80px] text-right overflow-hidden whitespace-nowrap">
              {props?.currencyemoji} {user.money}
            </div>
          </div>
