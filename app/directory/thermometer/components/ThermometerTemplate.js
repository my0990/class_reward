...
export default function ThermometerTemplate({ studentInfo, thermoInfo, role }) {
    ...
    const { userId, money, classData, profileNickname, teacher } = data;
    ...
    const temp = 23 + (sum / requireCurrency) * 3.3;
    console.log(data)
        ...
        return (
            <div className="flex justify-center items-center  flex-col">
                <div className="flex mt-[32px] w-[647px] max-[647px]:w-full  max-[647px]:mt-0 justify-between">
                    <h1 className="text-[2rem] font-bold text-orange-500 ">현재온도 - {(sum / requireCurrency).toFixed(1)}도</h1>
                    {role === 'teacher'
                        ? <button onClick={() => document.getElementById('my_modal_3').showModal()} className="hover:bg-gray-300 px-[8px] rounded-lg bg-white border-0 outline-none">
