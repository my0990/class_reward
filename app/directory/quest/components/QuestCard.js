...
export default function QuestCard({ data, studentCount,  setIsDetail, setQuestDetailData }) {

    const date = new Date(data.time);

    const onDetail = () => {
        setIsDetail(true);
        setQuestDetailData(data)
    }
    const onDelete = (e) => {
        e.preventDefault();
    ...
    return (
        <div>
            ...
            <div className="relative p-[16px] border-4 border-orange-500 rounded-xl mt-[24px] w-[100%] ">
                ...
                <div className="text-red-900 mb-[16px]">{data.questGoal}</div>
                ...
                <div className="flex justify-end ">
                    <div className="flex items-center">
                        ...
                        {/* <Link prefetch={false} href={`./questDetail/${teacherId}/${data._id}`} key={data.id}><button className="btn bg-orange-500 border-0 text-white">상세보기</button></Link> */}
                        <button onClick={onDetail} className="btn bg-orange-500 border-0 text-white">상세보기</button>
                    </div>
                </div>
            </div>
        </div>
